...
func NewSqliteDB[B BackendBucket](dir string, buckets ...B) (*SqliteDB, error) {
	...
	if _, err := os.Stat(dir); err == nil {
		if err := os.Chmod(dir, 0777); err != nil {
			fmt.Printf("couldn't Chmod: %s", dir)
			return nil, err
		}
	}
...
func (s *SqliteDB) String() string {
	return fmt.Sprintf("SqliteDB<%q>", s.Dir)
}
...
func (s *SqliteTx) backup() (string, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	tmpdir := os.TempDir()
	os.MkdirAll(tmpdir, 0755)
	backup := filepath.Join(tmpdir, fmt.Sprintf("etcd.sqlite-snap-%d", time.Now().Unix()))
	os.Remove(backup)
...
func (s *SqliteTx) WriteTo(w io.Writer) (n int64, err error) {
	...
	defer f.Close()
	fmt.Printf("logDebug: after WriteTo SqliteTx.size = %d\n", s.Size())
	return io.Copy(w, f)
}
...
func (s *SqliteTx) CopyDatabase(lg *zap.Logger, dst string) (err error) {
	...
	if _, err := s.WriteTo(dest); err != nil {
