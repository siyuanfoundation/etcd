...
for ((i = 1; i <= n; i++ )); do
  ...
  $benchmarkBin put --total=${totalKeys} --conns=3 --clients=3 --key-size=128 --val-size=1024 --key-space-size=100000000 > "${outputDir}/${backendType}_put_benchmark_results_iter_${i}.log"
  ...
  $benchmarkBin range a b --total=${rangeTotal} --conns=3 --clients=3 > "${outputDir}/${backendType}_range_benchmark_results_iter_${i}.log"
