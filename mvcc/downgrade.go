...
func downgradeMetaBucket(lg *zap.Logger, tx backend.BatchTx) {
	if lg != nil {
		lg.Info(
		...
		plog.Printf("downgrade meta bucket: remove keyes [%s, %s]", string(confStateKeyName), string(termKeyName))
	}
